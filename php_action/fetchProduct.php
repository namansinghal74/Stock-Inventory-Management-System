...
$sql = "SELECT product.product_id, product.product_name, product.product_image, product.brand_id,
 		product.categories_id, product.quantity, product.rate, product.active, product.type, product.status, 
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[8] == 1) {
 		// activate member
 		$type = "<label class='label label-success'>Returnable</label>";
 	} else {
 		// deactivate member
 		$type = "<label class='label label-danger'>Non Returnable</label>";
 	} // /else
